...
;(function($){
	...
	$(function(){
		...
		$.fn.imgLoading = function(_settings){
			...
			var _this = this;
			var handler = function(){
				$(settings.obj).append("<span class='number' data-number='0'>0</span> <span class='percent'>%</span>");
...
function fanswoo(){
	...
	this.concat_prepend = function(arr, arr2){
		var arr = arr || [];
		var arr2 = arr2 || [];
		var concat_arr = fanswoo.transfer(arr2);
		concat_arr.reverse();
		for( concat_key in concat_arr )
		{
			arr.unshift(concat_arr[concat_key]);
		}
		return arr;
	}

	this.concat_append = function(arr, arr2){
		var arr2 = arr2 || [];
		for( arr2_key in arr2 )
		{
			arr.push(arr2[arr2_key]);
		}
		return arr;
	}

	this.transfer = function(arg)
	{
		var return_value = [];
		for( key in arg )
		{
			return_value[key] = arg[key];
		}
		return return_value;
	}

	this.message_show = function(str, second, url, js)
	{
		var second;
		var js;
		if( second >= 2)
		...
		$("[fanswoo-message_show_clone]").clone().removeAttr("fanswoo-message_show_clone").attr("fanswoo-message_show", 'true').css('display', 'block').prependTo('body');
		setTimeout(function(){
			var width_all = $('[fanswoo-message_show]').widthAll();
			$('[fanswoo-message_show]').css('margin-left', - (width_all) / 2);
			var animate_time = second * 1000 - 300;
			setTimeout(function(){
		...
		$("[fanswoo-message_show] .message_content").html(str);
	...
	this.polling_catch_event = function(arg)
	{
		var obj_class = arg.obj_class;
		var watching_ids_arr = arg.watching_ids_arr;
		var comply_function = arg.comply_function;
		var _this = this;
		...
		$.ajax({
            ...
            type: 'POST',
            data: {
            	watching_ids_arr: watching_ids_arr,
            	obj_class: obj_class
            }
        })
    	.done(function(response){
	    	...
	    	.done(function(response2){
	    		...
	    		if( response2 == '' )
	    		{
					_this.polling_catch_event(arg);
	    		}
	    		else
	    		{
		    		var change_ids_arr = response2.split(',');
					comply_function(change_ids_arr);
	    		}
