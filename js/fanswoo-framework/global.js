...
;(function($){
	...
	$(function(){
		...
		$.fn.fs_ajax_submit = function(callback)
		{
			$this = $(this);
		...
		$(':submit').fs_ajax_submit(function(response){
			// if( fanswoo.is_json(response) )
			// {
			// 	console.log(response);
			// 	$this_dom = $('[abc_clone]').clone().attr('abc').removeAttr('abc_clone');
			// 	$this_dom.text(response.obj.title);
			// 	$this_dom.text(response.obj.name);
			// }
		});
	    ...
	    $(document).on('click', "[fs-submit-send-type!='post']:submit", function(event){
	        ...
	        $.ajax({
	            ...
	            success: function(response){
	                ...
	                {
	                	alert('未獲取標準fansWoo Message JSON，請開啟 console.log 查看錯誤訊息');
		            	console.log(response);
		            	return false;
	                  //   fanswoo.message_show(
...
function fanswoo(){
	...
	this.message_show = function(str, second, url)
	{
		var second;
		if( second >= 2)
		{
