...
<div class="contentBox contentTablelist allWidth">
        ...
        <div class="spanLineTableContent">
            ...
            <div class="spanLine tablelist">
                <?php echo form_open("admin/$child1_name/$child2_name/$child3_name/{$child4_name}_post/") ?>
                    ...
                    <?endif?>
                    <div class="spanLineLeft text width100">
                        <input type="number" class="text" style="margin-left:-6px;" value="<?=!empty($search_pagerid)?$search_pagerid:''?>" name="search_pagerid" placeholder="請填寫ID">
                    </div>
                    <div class="spanLineLeft text width400">
                        <input type="text" class="text" style="margin-left:-6px;" value="<?=!empty($search_title)?$search_title:''?>" name="search_title" placeholder="請填寫頁面標題">
                    </div>
                    <div class="spanLineLeft text width150">
            ...
            <div class="spanLine tablelist">
                <div class="spanLineLeft checkbox">
                    <input type="checkbox" name="pagerid_arr[]" value="<?=$value_Pager->pagerid?>" class="check">
                </div>
                <div class="spanLineLeft text width100">
                    <?=$value_Pager->pagerid?>
                </div>
                <div class="spanLineLeft text width400">
                    <a href="admin/<?=$child1_name?>/<?=$child2_name?>/<?=$child3_name?>/edit/?pagerid=<?=$value_Pager->pagerid?>"><?=$value_Pager->title?></a>
                </div>
                <div class="spanLineLeft text width150">
                ...
                <div class="spanLineLeft width100 tablelistMenu">
                    <a href="admin/<?=$child1_name?>/<?=$child2_name?>/<?=$child3_name?>/edit/?pagerid=<?=$value_Pager->pagerid?>">編輯</a>
                    <span class="ahref" onClick="fanswoo.check_href_action('確定要刪除嗎？', 'admin/<?=$child1_name?>/<?=$child2_name?>/<?=$child3_name?>/delete/?pagerid=<?=$value_Pager->pagerid?>&hash=<?=$this->security->get_csrf_hash()?>');">刪除</span>
                </div>
        	</div>
            <?endforeach?>
    ...
    <?if(!empty($PagerList->obj_arr[0]->pagerid)):?>
